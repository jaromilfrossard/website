...
Indeed using this formula the random structure allows covariance between the random "slopes" and a unique variance for earch random slopes. This structure produce $1 + (a - 1) + (b -1)$ variance parameter ( `r 1 + a + b` in our case) and $a(a-1)/2 + b(b-1)/2$ covariance paramters ( `r a*(a-1)/2 + b*(b-1)/2` in our case). However the assumed model has only 3 parameters of variance (+ 1 confounded with the error term).

The solution available in the lme4 package is to benefits from the modular construction of the lmer formula. Its allows of full flexibility on the random structure you want to estimate but it need a good understandinf of the estimation of the model (and some programming skills).
