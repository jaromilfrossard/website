...
The underlying model of the repeated measures ANOVA can be written as a mixed linear models. However mixed models packages like lme4 [@bates_fitting_2015] makes the estimation of such model not intuitive. For a 2 repeated measures ANOVA with 2 within factors, we write the following model : 
...
where $\mathbf{y}$ is the vector of response, $\mathbf{X}$ and $\mathbf{Z}$ are respectively the fixed and random design. $\mathbf{\beta}$ is the vector of fixed parameters, $\mathbf{\gamma}$ are the random parameters and $\mathbf{\epsilon}$ is the error term. @christensen_plane_2010 give a more detailed explanation on the link between repetead measure ANOVA and LMM. 
...
The aov() function compute type 1 sums of square. However in our fully balanced design, it will provide the same results than the type 3. The output gives F test and exact p-values under the assumptions. Because we cannot be trust the assumption to be fully fulfilled, the Greenhouse-Geisser or Huynh-Feldt correction are often used to compute p-values. In the case you want to those correction or if you have a unbalanced design (and you want to use type 3 sum of squares) I recommand to use the afex package [@singmann_afex:_2017].
 
The formula of aov() should be written using $+ Error(subject/(within*factors))$ to specify the within factors in the model. 
...
The formula of lme4 allows user to specify several type of random design. However it handels the factor in an un-intuitive way and if the random structure is miss-specified the estimation will fail to converge.

A first approach is to think of the two random effects $(\pi\alpha)_{ij}$ and $(\pi\psi)_{ik}$ as random slopes and write those random effects as $+ (factor|subject)$. The ANOVA model does not assume covariance between random slope and random intercept and ee can implemente this feature using the double bar symbol $( \cdot || \cdot )$ in the formula. Morover the higher level interaction is confounded with the error and is not estimable. This let us with the following formula :

```{r, echo=T, eval=F}
y ~ alpha * psi + (alpha + psi || id)
```

However this formula does not match the assumed model and estimate more parameters than the assumed model does. We will first estimated different variances for each random slopes in the same "group" ($Var \left[  (\pi\alpha)_{ij}\right] = Var\left[  (\pi\alpha)_{i'j'}\right]$) and secondely we estimate covariance between the random "slopes". Using this formula the assumed covariance structure is composed of $1 + (a - 1) + (b -1)$ variance parameters ( `r 1 + a + b` in our case) and $a(a-1)/2 + b(b-1)/2$ covariance paramters ( `r a*(a-1)/2 + b*(b-1)/2` in our case). However the ANOVA model has only 3 parameters of variance (+ 1 confounded with the error term) and 0 covariance parameter.
...
The solution available in the lme4 package [@bates_fitting_2015] is to benefits from the modular construction of the lmer formula. Its allows a large flexibility on the random structure you want to estimate but it need a good understanding of the estimation of the model (and some programming skills).
...
## Completed lmerMod object.
summary(m2)

```

Based on this formula we find a way to estimate the same model than the ANOVA assume using lmer. The two notation assumed different random structure and can be resumed :

1. $+ (A|id)$ assumes a $A\sim \mathcal{N}(0,\Omega))$
2. $+ (1|A:id)$ assumes $A\sim \mathcal{N}(0,I\sigma_A^2)$

The first solution has the advantage to reduce the number of assumption on the covariance structure. The cost of this freedom is all the parameters in the $\Omega$ matrix, that increase in $n^2$ with the number of levels of $A$. The second solution puts more assumption but reduce the number of parameters and gives more chance to the algorithm to converge. This solution may aslo be used with multiple random effects, like the subject and the item and their interaction. However finding an algorithm that converge gives us no information about the true model. If we estimate a model without covariances but the data are generated with covariance, the inference should be wrong.
