...
Next we construct the headset by assembling several spheres representing each electrode. We will display the tests of the 40th time-point (78 ms). Electrodes in metallic-red are inside a significant cluster, in metallic grey are inside a non-significant cluster and the transparent one are not part of a cluster. Using [rayrender](http://www.rayrender.net/), we construct all 64 electrodes in the appropriate position, using a for loop and, using the group_objects() function, we group them into a unique 3D object that we can manipulate. By trial and error, we found the best translation, rotation and re-scale of the headset to fit it on the head. 
...
Finally we plot 4 different points of view in order to have preview of the effect. The computation may take a long time so you try lower quality setting first.
